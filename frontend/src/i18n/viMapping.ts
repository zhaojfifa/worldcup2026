...
export function reasonBulletsVi(m: Match): string[] {
  if (m.features && m.features.length) {
    return m.features.slice(0, 3).map((f) => {
      const label = featureLabelViMap[f.label] ?? f.label;
      return `${label}: ${f.value >= 0 ? 'đóng góp tích cực +' : 'trừ rủi ro '}${f.value}%`;
    });
  }
  ...
  const bullets = [
    ...
    `Chỉ số tin cậy ${Math.round(m.confidence)}, ${riskText}`,
  ];
  const tr = topRisk(m);
  if (tr) bullets.push(`Điểm chú ý: ${noteVi(tr)}`);
