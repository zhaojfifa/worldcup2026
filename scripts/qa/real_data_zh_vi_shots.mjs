#!/usr/bin/env node
/**
 * Real Data zh+vi verification screenshots (zero-dep CDP). Requires dev server +
 * the QA_RECAP_TEMP finished mock matches (reverted after capture).
 * Output: docs/qa_screenshots/real_data_zh_vi_verification/
 */
import { spawn } from 'node:child_process';
import { mkdirSync, writeFileSync } from 'node:fs';
import { setTimeout as sleep } from 'node:timers/promises';
const BASE = process.argv[2] || 'http://localhost:5173';
const OUT = 'docs/qa_screenshots/real_data_zh_vi_verification';
...
// zh current
await nav('/?lang=zh'); await shot('home-zh-390.png');
await nav('/detail?lang=zh'); await shot('detail-zh-current-390.png');
await nav('/report?lang=zh'); await shot('report-zh-current-390.png');
// vi current
await nav('/?lang=vi'); await shot('home-vi-390.png');
await nav('/detail?lang=vi'); await shot('detail-vi-current-390.png');
await nav('/report?lang=vi'); await shot('report-vi-current-390.png');
await nav('/community?lang=vi'); await shot('community-vi-390.png');
// vi finished historical recap (click recap row → detail, then report)
...
// zh finished recap
await nav('/?lang=zh'); await evalJs(`(()=>{const r=document.querySelector('.recap-row');if(r)r.click();return!!r;})()`); await sleep(900);
await shot('detail-zh-finished-recap-390.png');
