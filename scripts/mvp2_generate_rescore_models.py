...
def build_user(sk, language):
    ...
    return (
        ...
        + "\n\nReturn ONLY one JSON object:\n"
          ...
          '"pre_match_risk_level":"<short>","score_range_before":"<persona reference band: zh 俅哥给出的赛前参考区间… / vi khoảng tham khảo trước trận của Tiên Tri… / my Football Oracle ၏ ပွဲကြို ရည်ညွှန်းအပိုင်းအခြား…>",'
          ...
          "public_teaser or pre_match_lean); language %s%s; NO URLs; no betting words (kèo/cửa trên/cửa dưới/盘口/赔率/လောင်းကစား...); NEVER the words 模型/mô hình/မော်ဒယ်/ScoutScore/AI in copy — speak ONLY as the persona; "
          "no 'we lack data' phrasing (use 赛前盲区/临场待确认/cần xác nhận trước giờ bóng lăn/ပွဲမစခင် ပြန်စစ်ရမည့်အချက်); gaps are product features, "
          "not apologies. JSON only." % (persona.split("（")[0], language,
                                          " — ZERO Han characters" if language in ("vi-VN", "my-MM") else "")
    )
...
def check(obj, language):
    ...
    if language == "vi-VN":
        ...
        if HAN.search(blob):
            errs.append("my Han chars present")
        if "Oracle" not in blob:
            errs.append("my persona Football Oracle missing")
    else:
        if "俅哥" not in blob:
...
def generate(provider, fid, language, keys):
    ...
    obj.update({"fixture_id": fid, "language": language,
                "voice": GEN.VOICE.get(language, "tientri_v2"), "llm_provider": provider,
