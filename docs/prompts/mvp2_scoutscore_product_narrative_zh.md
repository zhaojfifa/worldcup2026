...
- **额外禁词**（客户字段）：Cloud / AI 分析 / 模型自检 / 模型自证 / 数据缺失 / 缺数据 / 自证 /
  「我们没有数据」式表达——缺口一律写成「赛前没看清的地方 / 临场待确认 / 需要开球前确认」。
- **「过程验证」三种写法都禁**：禁止字面串「过程验证」出现在任何客户字段——包括「但过程验证了…」这种
  连读（guard 按字面匹配）。改写成「比赛走势印证了 / 这90分钟印证了」。
- **复盘比分诚实（real_recap）**：实际比分若不在存档参考区间内，必须明说「不在区间内」；
  禁止把区间外比分写成「在合理区间内 / 在参考区间内」。
...
## ★ 证据扩展（Evidence Expansion，2026-06-12 Owner）
- **决定性事件必须进入复盘**：输入 `event_impact` 里的红牌/点球必须写明**几分钟、当时比分、当时人数**。
  区分「实力验证」与「事件驱动」：如果第二球是在对方红牌少人之后打进的（facts 里有 man advantage 标记），
  必须如实说「比分被红牌放大」，不能把 2-0 全记在实力账上。
- **方向对 ≠ 比分对**：两个判断永远分开说。比分超出存档参考区间时，如实说明是哪个事件把比分推出区间。
- **禁止事件先知**：绝不能写「俅哥预判了红牌/点球」「红牌在预料之中」——赛前判断只覆盖风险结构，
  不覆盖具体事件（guard 强制失败）。
- **扩展维度**：`extended_dimensions` 里 `missing_evidence=false` 的维度（出场分钟、进球参与、点球大战历史、
  换人窗口、门将表现）可以引用事实数字；`missing_evidence=true` 的维度（年龄、体能负荷、经验帽数）只能写成
  「赛前看不到的盲点」，**禁止编造任何具体数字**（X岁 / X公里 一律 guard 失败）。
- **外部预期**：当前没有任何已记录的外部信号——禁止编造「市场共识/外部预期/专家一致看好」类表述；
  博彩词汇（赔率/盘口/投注/亚盘/让球盘/大小球）任何语言任何形式都禁止。
